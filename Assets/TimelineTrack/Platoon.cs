...
public class Platoon : MonoBehaviour
{
    ...
    {
        if(command.commandType == AICommand.CommandType.GoTo)
        {
            tempPositions = GetFormationPositions(command.targetDestination.position);
            for (int i = 0; i < units.Count; i++)
            {
                if (units.Count > 1)
                {
                    ...
                    //tempPositions[i] += command.targetDestination.position;

                    Debug.Log(tempPositions[i].ToString());
                    command.destination = tempPositions[i];
                }
                else
                {
                    command.destination = command.targetDestination.position;
                }
    ...
    public void AddUnit(BaseScript unitToAdd)
    {
    ...
    public int AddUnits(BaseScript[] unitsToAdd)
    {
    ...
    public bool RemoveUnit(BaseScript unitToRemove)
    {
    ...
    private void UnitDeadHandler(BaseScript whoDied)
    {
