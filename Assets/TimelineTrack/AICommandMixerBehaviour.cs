...
public class AICommandMixerBehaviour : PlayableBehaviour
{
	...
	private void ProcessEditModeFrame(Playable playable)
	{
		...
		for (int i = 0; i < inputCount; i++)
		{
			...
			finalPositions = trackBinding.GetFormationPositions(input.targetDestination.position);
	...
	private void ProcessPlayModeFrame(Playable playable)
	{
		...
		for(int i = 0; i < inputCount; i++)
		{
			...
			if(inputWeight > 0f)
			{
				...
				{
					AICommand c = new AICommand(input.commandType, input.targetPosition, input.targetUnit,input.targetDestination);
