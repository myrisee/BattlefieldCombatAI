...
{
    public MeshRenderer TrackLeft, TrackRight;
    public Material TrackMaterial;

    private float currentMaxSteeringAngle, currentSteeringSpeed;    
    ...
    private void Start()
    {
        ...
        // Compute turn radius
        turnRadius = wheelbase / Mathf.Sin(MaxSteeringAngle * Mathf.Deg2Rad) /2f;

        Debug.Log(gameObject.name + ": turning radius = " + turnRadius + ", wheelbase = " + wheelbase);

        // Instantiate material instanced for tracks (each will have its own X-Offset)
        TrackLeft.material = new Material(TrackMaterial);
        TrackRight.material = new Material(TrackMaterial);

        currentMaxSteeringAngle = MaxSteeringAngle;
    ...
    public override void SetSteeringAngle(float input)
    {
        ...
        {
            currentMaxSteeringAngle = Mathf.Lerp(MaxSteeringAngle, 10, speed / TopSpeed);
            currentSteeringSpeed = Mathf.Lerp(SteeringSpeed, 10, speed / TopSpeed);
            // Apply steering input
            steeringAngle = Mathf.Clamp(steeringAngle + input * SteeringSpeed * Time.deltaTime, -currentMaxSteeringAngle, currentMaxSteeringAngle);
        }
    ...
    {
        // Compute offsets based on vehicle speed
        float leftTrackOffset, rightTrackOffset;
        leftTrackOffset = rightTrackOffset = -speed * Time.deltaTime * 0.1f;
        // Restrict offsets based on turning angle to simulate how tracks work 
        leftTrackOffset *= (1-steeringAngle / -currentMaxSteeringAngle);
        rightTrackOffset *= (1-steeringAngle / currentMaxSteeringAngle);


        TrackLeft.material.mainTextureOffset = new Vector2(TrackLeft.material.mainTextureOffset.x + leftTrackOffset, 0);
        TrackRight.material.mainTextureOffset = new Vector2(TrackRight.material.mainTextureOffset.x + rightTrackOffset, 0);
    }
}
